...
function updateOrder($pdo, $customer_name, $customer_email, $order_id, $updated_by) {
    $sql = "UPDATE orders SET customer_name = ?, customer_email = ?, updated_by = ? WHERE order_id = ?";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$customer_name, $customer_email, $updated_by, $order_id]);
    if ($executeQuery) {
        return true;
    }
}
